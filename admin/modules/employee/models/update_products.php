<?php 
class EmployeeModelsUpdate_products extends FSModels
{
	var $limit;
	var $prefix ;
	function __construct()
	{
		...
		$this -> table_name = 'fs_update_products';
			// config for save
	...
	function save($row = array(), $use_mysql_real_escape_string = 0) {
		...
		if($id){
			...
			$check = $this->get_record('type = 2 AND record_id = '.$id,'fs_list_import_products');
			// dd($row2);
			if(!empty($check)){
