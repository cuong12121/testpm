...
//			$select_cat = FSInput::get('select_cat');
	...
	//		$all_groups = $model->getUserGroupsAll();
		...
		function add()
		{
			...
			$users = $model-> get_records('published = 1','fs_users');
			$groups = $model -> get_records('1=1','fs_users_groups');
		...
		function save()
		{
			...
			if($cid)
			{
				setRedirect(FSRoute::_('index.php?module=users&view=profile'),FSText :: _('Saved'));	
...
function view_warehouses($controle,$id){
	$model = $controle -> model;
	$user = $model->get_record('id = ' .$id,'fs_users','id,wrap_id_warehouses');
	$html = "";
	if($user-> wrap_id_warehouses){
		$user-> wrap_id_warehouses = substr($user-> wrap_id_warehouses, 1,-1);
		$warehouses = $model->get_records('id IN('.$user-> wrap_id_warehouses.')','fs_warehouses','id,name');
		if(!empty($warehouses)){
