...
class NewsModelsNews extends FSModels {
	...
	function __construct() {
		...
		$this -> arr_img_paths = array(array('resized2',270,151,'cut_image'),array('resized',400,224,'cut_image'),array('small',200,112,'cut_image'),array('large',1000,560,'cut_image'));
		...
		$cmonth = date ( 'm' );
		$cday = date ( 'd' );
		$this->img_folder = 'upload_images/images/news/' . $cyear . '/' . $cmonth . '/' . $cday;
		$this->check_alias = 0;
	...
	function setQuery() {
		...
		$check_group = $this->get_record('id = ' . $_SESSION['ad_userid'],'fs_users');
