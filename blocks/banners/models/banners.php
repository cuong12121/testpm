<?php 
	class BannersBModelsBanners
	{
		...
		function getList($category_id){
			...
			if($ccode){
				if($module == 'products'){
					$where .= 'AND (products_categories_alias like "%,'.$ccode.',%" OR products_categories_alias IS NULL) ';
				}else if($module == 'news'){
					$where .= 'AND  (news_categories_alias like "%,'.$ccode.',%" OR news_categories_alias IS NULL) ';
				}else{
				}
			}
			// Itemid
