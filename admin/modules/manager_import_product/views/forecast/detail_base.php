...
<?php
		...
		if(!empty($product) && empty($data)){
			TemplateHelper::dt_edit_text(FSText :: _('Tên sản phẩm'),'name',@$product -> name,'',100,1,0);
			TemplateHelper::dt_edit_text(FSText :: _('Mã sản phẩm'),'code_product',@$product -> code,'',100,1,0);
			TemplateHelper::dt_edit_text(FSText :: _('Số lượng'),'count',@$amount,'',100,1,0);
		}else{
...
<?php
		TemplateHelper::dt_edit_text(FSText :: _('Link nhà cung cấp'),'link',@$data -> link,'',100,5,1);
		...
		TemplateHelper::dt_edit_text(FSText :: _('Tối ưu ([Giá mới + RMB mới + PVC] - [Giá cũ + RMB trước +PVC trước])'),'toi_uu',@$data -> toi_uu,'',100,1,0);
	    ...
	    TemplateHelper::dt_edit_text(FSText :: _('Ngày dự kiến về hàng'),'ngay_du_kien_ve_hang',check_empty_date(@$data-> ngay_du_kien_ve_hang),'',60,1,0);
...
<script  type="text/javascript" language="javascript">
    $(function() {
        ...
        $( "#ngay_du_kien_ve_hang" ).datepicker({clickInput:true,dateFormat: 'dd-mm-yy',changeMonth: true, changeYear: true});
    });
</script>