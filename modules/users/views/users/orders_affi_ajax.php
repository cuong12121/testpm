...
<?php if(!empty($list_orders)){?>
	<div class="stastics">
		<div class="total_price">Tổng giá trị: <span class="price1"><?php echo format_money($total); ?></span></div>
		<div class="total_price">Tổng giá trị đơn hàng hoàn tất: <span class="price"><?php echo format_money($total2); ?></span></div>
		<?php if(empty($price_affi)) { ?>
			<div class="status_money">Bạn chưa nhận được thanh toán!</div>
		<?php } else { ?>
			<div class="status_money">Bạn đã nhận được thanh toán: <span class="price"><?php echo format_money($price_affi-> value); ?></span></div>
		<?php } ?>
	</div>
