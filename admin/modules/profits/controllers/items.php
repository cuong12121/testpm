<?php
	class ProfitsControllersItems extends Controllers
	{
		...
		function display()
		{
			...
			if(!empty($statics)){
				...
				foreach ($statics as $static_item){
					$tong_doanh_thu += $static_item-> gia_tri_don_hang;
					$tong_loi_nhuan += $static_item-> profit;
					$tong_gia_von += $static_item-> total_product_price;
				}
			}
		...
		function ajax_add_time_profits(){
			...
			$shop_code = FSInput::get('shop_code');
			$row = array();
			...
			$check = $model->get_record('go_time = "'.$row['go_time'] . '" AND to_time = "'.$row['to_time']. '" AND shop_code = "'.$shop_code.'"','fs_profits_shops');

			if(!empty($check)){
				$model->_update($row,'fs_profits_shops','id = '.$check->id);
				$rs = $check->id;
			}else{
				$rs = $model->_add($row,'fs_profits_shops');
			}
			if($rs){
				$data = array ('error' => false, 'messenger' => 'Tạo mốc thành công', 'link'=> URL_ADMIN.'profits/profist_shop/edit/'.$rs);
