...
// 		cuong:them
    ...
    //         return $maVanDonMatches[0]; // Output: <?php
        ...
        function convertContentCheck($content){
        	...
        	if(empty($b[0])){
        		preg_match_all('/[A-Z-0-9]{1}[A-Za-z0-9]{1}[A-Za-z0-9]{1}[A-Za-z0-9]{1}+\s*-\s*[A-Za-z0-9][A-Za-z0-9]+-+\s[A-Za-z0-9][A-Za-z0-9]+\s*-\s*[A-Za-z0-9][A-Za-z0-9][A-Za-z0-9]+\s*-\s*[A-Za-z0-9][A-Za-z0-9]+\s*-\s*[A-Za-z0-9][A-Za-z0-9][A-Za-z0-9]/', $content, $b);
        	}
		...
		function prints($value)
        {
            ...
            for($i=1; $i<3; $i++){
                ...
                foreach ($plarform as  $plarforms) {
                     ...
                     $query =  "SELECT id FROM fs_order_uploads AS a WHERE 1=1 AND warehouse_id = ".$i." AND house_id = ".$house_id." AND platform_id = ".$plarforms." AND date ='".date('Y-m-d')."' ORDER BY created_time DESC , id DESC";
