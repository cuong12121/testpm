<?php 
	class PackagesModelsPackage extends FSModels
	{
		var $limit;
		var $page;
			...
			if(!empty($ids)){
				...
				foreach ($ids as $id) {
					...
					if(!empty($data) && $data-> is_package != 1){
						...
						$product = $this->get_record('code = "'.$data-> product_code.'"','fs_products','id,price_pack');
						if(!empty($product)){
						...
						$update_package = $this-> _update($row,'fs_order_uploads_detail','id ='.$id);

						if($update_package){
							$this->minus_money($data); //trừ tiền của tài khoản
							$i++;
						}
