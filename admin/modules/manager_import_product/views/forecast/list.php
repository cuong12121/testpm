...
<?php
	...
	$prefix = 'manager_import_product_forecast_';
?>	
		...
		<table style="width: 100%;" id="dataTables-example" class="table table-hover table-striped table-bordered">
			...
			<tbody>
				<tr>
					...
					<td></td>
					<td></td>
					<td>Link nhà cung cấp</td>
					<td>Giá sản phẩm mới</td>
					<td>Giá RMB</td>
					<td>Giá sản phẩm cũ</td>
					<td>Giá RMB Cũ</td>
					<td>Phí vận chuyển mới</td>
					<td>Phí vận chuyển cũ</td>
					<td>Tối ưu</td>
					<td>Giá dự kiến về Kho</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				...
				<tr>
					...
					<td><div class="wrap_list_pr wrap_list_pr_link"><?php echo $item-> link ?></div></td>
					<td><?php echo format_money($item-> price_product_new) ?></td>
					<td><?php echo format_money($item-> price_rmb);?></td>
					<td><?php echo format_money($item-> price_product_old) ?></td>
					<td><?php echo format_money($item-> price_rmb_old) ?></td>
					<td><?php echo format_money($item-> price_ship) ?></td>
					<td><?php echo format_money($item-> price_ship_old) ?></td>
					<td><?php echo format_money($item-> toi_uu) ?></td>
					...
					<td><div class="wrap_list_pr wrap_list_pr_note"><?php echo $item-> note;?></div></td>
					<td><div class="wrap_list_pr wrap_list_pr_propose"><?php echo $item-> propose;?></div></td>
					<td><?php echo @$array_import[$item-> is_import] ?></td>
					<td><?php echo $item->date_to_ha_noi ? date('d/m/Y',strtotime($item->ngay_du_kien_ve_hang)) : '' ?></td>
...
<style type="text/css">
	.wrap_list_pr_propose,.wrap_list_pr_note{
		width:350px !important;
	}
	.wrap_list_pr_propose,.wrap_list_pr_link{
		width:200px;
	}
	.note-top{
		color: red;
