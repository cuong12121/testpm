...
//				$where .= '  AND is_hot = 1 ';	
	...
	function get_list_product($sale_id) {
		$fs_table = FSFactory::getClass ( 'fstable' );
		$query = " SELECT p.id, p.name ,p.sale_count, p.image , s.price, p.types ,s.price_old , p.alias, p.category_alias, p.category_id , p.is_new
		FROM " . $fs_table->getTable ( 'fs_sales_products' ) . " as s INNER JOIN ".$fs_table->getTable ( 'fs_products' ) ." as p ON s.product_id = p.id
		WHERE published = 1 AND sale_id = $sale_id
		";
		global $db;
		$sql = $db->query ( $query );
		$result = $db->getObjectList ();
	...
	function get_sale($type){
		...
		WHERE published = 1 AND started_time < '".$today_time ."' AND finished_time > '".$today_time."' AND type =".$type.  " ORDER BY ordering ASC";
		global $db;
		$sql = $db->query ( $query );
		$result = $db->getObject ();
		return $result;
		...
		FROM " . $fs_table->getTable ( 'fs_sales' ) . "
		WHERE published = 1 AND started_time > '".$today_time ."' AND type =".$type.  " ORDER BY started_time ASC, ordering ASC";
		global $db;
		$sql = $db->query ( $query );
		$result = $db->getObject ();
		return $result;
