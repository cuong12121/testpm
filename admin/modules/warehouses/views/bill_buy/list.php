...
<?php  
global $toolbar;
$toolbar->setTitle(FSText :: _('Danh sách nhập hàng') );
...
$filter_status['list'] = $this-> arr_status_show;

		//Loại sản phẩm
$filter_status_buy = array();
$filter_status_buy['title'] = FSText::_('Tình trạng nhập hàng'); 
$filter_status_buy['list'] = $this-> arr_status_buy;
...
$fitler_config['filter'][] = $filter_status_buy;
...
$list_config[] = array('title'=>'Trạng thái phiếu','field'=>'id','ordering'=> 1, 'type'=>'text','arr_params'=>array('function'=>'show_status'));

$list_config[] = array('title'=>'Tình trạng nhập hàng','field'=>'id','ordering'=> 1, 'type'=>'text','arr_params'=>array('function'=>'show_status_buy'));
...
<script>
	...
	function change_status_ajax(id, status){
		$.ajax({
			url: '<?php echo URL_ADMIN; ?>index.php?module=warehouses&view=bill_buy&task=change_status_ajax&raw=1',
			type : 'POST',
		...
		function change_status_buy_ajax(id, status){
		$.ajax({
			url: '<?php echo URL_ADMIN; ?>index.php?module=warehouses&view=bill_buy&task=change_status_buy_ajax&raw=1',
			type : 'POST',
			data: {id: id, status: status},
			success : function(data){
				$('.dataTable_wrapper').load(location.href+" div.dataTable_wrapper>*","");
