<?php if(!empty($list_video_review)){?>
	<div class="video_popup cls">
		<?php parse_str( parse_url($list_video_review[0] -> link, PHP_URL_QUERY ), $my_array_of_vars );
		...
		<div class="video_item">
			<div class="video_item_inner video_item_inner_has_img">
				<?php 
				if(!empty($list_video_review[0] ->image)){
					echo set_image_webp($list_video_review[0] ->image,'large','Video '.$data->name,'video lazy',1,'link-video='.$video); 
				}else{
					?>
			...
			<div class="video_item_inner_has_yt hide"></div>
		</div>
	</div>
	<div class="list_video_popup">
		<?php $iv=0; foreach($list_video_review as $item){?>
			<?php if(!$item -> link) continue;?>
			<?php parse_str( parse_url($item -> link, PHP_URL_QUERY ), $my_array_of_vars );
			$id_video = $my_array_of_vars['v']; 
			$video = 'https://www.youtube.com/embed/'.$id_video;
			?>
			<div class="video_item">
				<div class="video_item_inner video_item_inner_has_img <?php if(!$iv) echo 'active'; ?>">
					<?php 
					if(!empty($item ->image)){
						echo set_image_webp($item->image,'resized','Video '.$data->name,'video lazy',1,'link-video='.$video); 
					}else{
						?>
						<img  class="video" src='<?php echo "https://i.ytimg.com/vi/".$id_video."/hqdefault.jpg" ?>' alt='<?php echo "Video ".$data->name;?>' link-video="<?php echo $video;?>" />
					<?php } ?>
