...
class Alepay {
    ...
    protected $URI = array(
        ...
        'requestCardLinkDomestic' => '/alepay-card-domestic/request-profile',
         'getlistBank' => '/checkout/v1/get-installment-info'
    );
    ...
    // TỰ DỊNH NGHIA
    public function getlistBank($amount,$currencyCode){
        ...
        $url = $this->baseURL[$this->env] . $this->URI['getlistBank'];
        $result = $this->sendRequestToAlepay($data, $url);
        if($result->errorCode == '000'){
            $dataDecrypted = $this->alepayUtils->decryptData($result->data,$this->publicKey);
...
// TỰ DỊNH NGHIA
    public function calculateFee($data){
        ...
        //     'bankCode'=>'SACOMBANK'
        // );
        $url = $this->baseURL[$this->env] . $this->URI['calculateFee'];
        $result = $this->sendRequestToAlepay($data, $url);
        if($result->errorCode == '000'){
            $dataDecrypted = $this->alepayUtils->decryptData($result->data,$this->publicKey);
            return json_encode($dataDecrypted);
