	...
	class OrderModelsUpload extends FSModels
	{
		...
		function setQuery()
		{
			...
			if($_SESSION['ad_groupid'] == 1){
				$where .= " AND user_id_manage_shop = ".$_SESSION['ad_userid'];
			}
		...
		function save_excel($row = array(),$result_id){
			...
			if($produt-> type_id == 9 && $produt-> code_combo && $produt-> code_combo !=''){ // là combo
				...
				foreach ($arr_code_combo as $combo_it){
					...
					$produt = $this->get_record('code = "'.$row['sku'].'"','fs_products');
					if(empty($produt)){
						$this -> _remove('id  = '.$result_id,'fs_order_uploads');
						$this -> _remove('record_id  = '.$result_id,'fs_order_uploads_detail');
						setRedirect($link,FSText :: _('Mã sản phẩm '.$row['sku'].' nhập trong mã combo cha không tồn tại, vui lòng kiểm tra lại!'),'error');
					}
					...
					if($data-> platform_id == 1){ // check để tính toán số lượng cho sàn lazada
						$check_count = $this->get_record('product_id ='.$produt->id.' AND lazada_sku = "'.$row['lazada_sku'].'" AND sku = "'.$row['sku'].'" AND record_id = '.$result_id,'fs_order_uploads_detail');
				...
				if($data-> platform_id == 1){ // check để tính toán số lượng cho sàn lazada
					$check_count = $this->get_record('lazada_sku = "'.$row['lazada_sku'].'" AND sku = "'.$row['sku'].'" AND record_id = '.$result_id,'fs_order_uploads_detail');
					if(!empty($check_count)){
						$row3 = array();
		...
		function prints($value)
		{
			...
			if(!empty($ids))
			{
				...
				if(!isset($_SESSION[$this -> prefix.'filter0']) || $_SESSION[$this -> prefix.'filter0'] == 0 || !isset($_SESSION[$this -> prefix.'filter1']) || $_SESSION[$this -> prefix.'filter1'] == 0 || !isset($_SESSION[$this -> prefix.'filter2']) || $_SESSION[$this -> prefix.'filter2'] == 0 ){
					setRedirect($link,FSText :: _('Vui lòng chọn lọc khung giờ, kho, sàn trước khi in!'),'error');
				}
				...
				foreach ($list as $item){
					...
					$list_detail = $this->get_records('record_id = '.$item->id,'fs_order_uploads_detail','DISTINCT find_pdf','sku_fisrt ASC,ABS(sku_fisrt),color ASC,ABS(color),size ASC,ABS(size)');
					// printr($list_detail);
				...
				//tìm số thứ tự in
				$list_detail_soft = $this->get_records('record_id IN ('.$str_ids.')','fs_order_uploads_detail','DISTINCT find_pdf','sku_fisrt ASC,ABS(sku_fisrt),color ASC,ABS(color),size ASC,ABS(size)');
				...
				$j = 1;
				$name_pdf = "";
				$get_list_page_pdf = $this->get_records('record_id IN ('.$str_ids.')','fs_order_uploads_page_pdf','id,file_pdf,record_id,code,find_pdf','ordering ASC');

				// printr($get_list_page_pdf);
				...
				foreach ($get_list_page_pdf as $item_page_pdf){
					...
					if(!$item_page_pdf-> find_pdf || $item_page_pdf-> find_pdf ==''){ //không biết phiếu này của dòng nào excel
						$err = 1;
						continue;
