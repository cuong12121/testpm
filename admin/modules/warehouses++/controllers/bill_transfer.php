...
function show_status2($controle,$id){
	...
	if(!empty($controle-> arr_status2_action[$status])) {
		...
		$txt .= '<br>';
		$txt .= '<a href="javascript:void(0)" onclick="change_status('.$id.','.$controle-> arr_status2_action[$status]['value'].')">'.'<i class="fa fa-arrow-right"></i>'.$controle-> arr_status2_action[$status]['text'].'</a>';
	} else {
		$txt = $controle-> arr_status2[$status];
	}
