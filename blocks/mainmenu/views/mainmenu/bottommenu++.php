	...
	foreach ($list as $item) {
		...
		if($item -> link)
			if($item->level==0){
				...
				$html .= "	<label class = 'title_bottom label_link' for='item_".$item->id."' data-id='item_".$item->id."'></label>";
				...
				if($item->level==0){
					$html .= "	<span class='click-mobile' data-id='menu-sub".$k."'></span>";
					$html .= "	<input type ='checkbox' id='item_".$item->id."'/>";
					$html .= "	<label class='title_bottom label_nolink' for='item_".$item->id."' data-id='item_".$item->id."'>";
				}else{
					$html .= "	<span>";
				}
				$name   = $item -> name;
