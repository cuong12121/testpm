<?php
	...
	class OrderModelsUpload extends FSModels
	{
		...
		function prints($value)
		{
			...
			// $str_ids = '2527,2525';
			// $list_detail_soft = $this->get_records('record_id IN ('.$str_ids.') AND sku_fisrt IS NOT NULL','fs_order_uploads_page_pdf','id,content,find_pdf,ordering','sku_fisrt ASC,ABS(sku_fisrt),sku_last ASC,ABS(sku_last)');
			...
			if(!empty($ids))
			{
				...
				if($_SESSION[$this -> prefix.'filter2'] == 1){ //tìm số thứ tự cho riêng lazada
				...
				}else{ //tìm số thứ tự in các sàn còn lại
