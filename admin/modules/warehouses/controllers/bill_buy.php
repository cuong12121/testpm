...
class WarehousesControllersBill_buy extends Controllers
{
	function __construct()
	{
		...
		if($check_permission_status_warehouses) {
			...
			$this-> arr_status1 = array('1' => 'Phiếu tạm','2'=>'Chờ duyệt');
			$this-> arr_status = $this-> arr_status1;
		...
		$this-> arr_status_buy = array('1' => 'Đặt cọc','2'=>'Đặt hàng','3'=>'Có hàng', '4'=> 'Đã nhập kho', '5' => 'Hủy');

		$this-> arr_status_buy_action = array('0'=> array('value' => '1', 'text' => 'Đặt cọc'),'1'=> array('value' => '2', 'text' => 'Đặt hàng'),'2'=> array('value' => '3', 'text' => 'Có hàng'),'3'=> array('value' => '4', 'text' => 'Nhập kho'));
	...
	function display()
	{
		...
		$breadcrumbs = array();
		$breadcrumbs[] = array(0=>'Nhập hàng', 1 => '');	
		global $tmpl,$check_permission_status_warehouses;
		$tmpl->assign ( 'breadcrumbs', $breadcrumbs );
		$tmpl->assign ( 'seo_title', 'Danh sách nhập hàng');
	...
	function add()
	{
		...
		$breadcrumbs = array();
		$breadcrumbs[] = array(0=>'Danh sách nhập hàng', 1 => 
			FSRoute::_("index.php?module=".$this-> module."&view=".$this-> view));	
		global $tmpl;
		$tmpl->assign ( 'breadcrumbs', $breadcrumbs );

		$tmpl->assign ( 'seo_title', 'Nhập hàng');
	...
	function print(){
		...
		global $tmpl;
		// $tmpl->assign ( 'seo_title', 'In phiếu Nhập hàng');
	...
	function change_status_buy_ajax(){
		$model = $this -> model;
		$id = FSInput::get('id');
		$data = $model-> get_record('id = '.$id,$model-> table_name);
...
function show_status_buy($controle,$id){
	$model = $controle-> model;
	$data = $model-> get_record('id = '.$id,$model-> table_name,'*');
	$status = $data-> status;
	...
	// $txt = $controle-> arr_status_buy[$status_buy];

	if(!empty($controle-> arr_status_buy_action[$status_buy])) {
		$txt = $controle-> arr_status_buy[$status_buy];
		if($check_permission_status_warehouses) {
			$txt .= '<br>';
			$txt .= '<a href="javascript:void(0)" onclick="change_status_buy_ajax('.$id.','.$controle-> arr_status_buy_action[$status_buy]['value'].')">'.'<i class="fa fa-arrow-right"></i>'.$controle-> arr_status_buy_action[$status_buy]['text'].'</a>';
		}
	} else {
		$txt = $controle-> arr_status_buy[$status_buy];
	}
...
function show_status($controle,$id){
	...
	if(!$controle-> arr_status_show[$status]) return;
	...
	if(!empty($controle-> arr_status_action[$status])) {
		$txt = $controle-> arr_status_show[$status];
		if($check_permission_status_warehouses) {
			$txt .= '<br>';
