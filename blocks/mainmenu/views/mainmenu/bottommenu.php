	...
	foreach ($list as $item) {
		...
		if($item -> link)
			if($item->level==0){
				...
				if($item-> nofollow) {
					$html .= "	<a class='title_bottom' rel='nofollow' href='".$link."' title='".$item -> name."'>";
				} else {
					$html .= "	<a class='title_bottom' href='".$link."' title='".$item -> name."'>";
				}
				
			}else{
				if($item-> nofollow) {
					$html .= "	<a rel='nofollow' href='".$link."' title='".$item -> name."'>";
				} else {
				...
				if($item->level==0){
					$html .= "	<span class='click-mobile' data-id='menu-sub".$k."'></span>";
					$html .= "	<span class='title_bottom' data-id='item_".$k."'>";
				}else{
					$html .= "	<span>";
				}
				$name   = $item -> name;
