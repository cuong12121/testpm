...
class ProductsModelsProducts extends FSModels {
	...
	function __construct() {
		...
		$this -> arr_img_paths_hot =array(array ('resized', 470, 220, 'resized_not_crop' ));
		$this -> arr_img_paths_banner = array(array ('resized', 300, 500, 'resized_not_crop' ),array('compress',1,1,'compress'));
		$this -> arr_img_paths_video_review = array(array('resized',400,300,'cut_image'),array('large',800,440,'cut_image'));
		...
		$this->image_watermark_pos = 7; //Đóng dấu nên ảnh
		$this->image_watermark_pos1 = 3;
		$this->load_params ();
	}
	...
	function load_params() {
		...
		$this->arr_img_paths_other = array (array ('large', 1020, 680, 'resize_image' ), array ('small', 150, 150, 'cut_image' ) );
	}
	/*
	...
	function setQuery() {
		...
		if (isset ( $_SESSION [$this->prefix . 'sort_field'] )) {
			...
			if(!empty($_SESSION [$this->prefix . 'filter1']) && $_SESSION [$this->prefix . 'filter1'] > 0){
				...
				if ($sort_field && $sort_field !='amount_can_by'){
					$ordering .= " ORDER BY $sort_field $sort_direct, created_time DESC, id DESC ";
				}
			}
		...
		if(!empty($_SESSION [$this->prefix . 'filter1']) && $_SESSION [$this->prefix . 'filter1'] > 0){
			...
			if ($filter) {
				$where .= ' AND warehouses_id  =' . $filter . '';
			}

			$query = " SELECT a.*,b.* FROM " . $this->table_name . " as a LEFT JOIN fs_warehouses_products_can_by as b ON a.id = b.product_id WHERE 1=1" . $where . $ordering;
		}else{
			// $query = " SELECT a.*
			// FROM 
			...
			$query = " SELECT a.*,b.* FROM " . $this->table_name . " as a LEFT JOIN fs_warehouses_products_total as b ON a.id = b.product_id WHERE 1=1 " . $where . $ordering;

			// $query = "select `a`.`product_id` AS `product_id`,sum(`a`.`amount`) AS `total`,sum(`a`.`amount` - `a`.`amount_hold`) AS `total_can` from `fs_warehouses_products` `a` WHERE `a`.`product_id` = 37895918 group by `a`.`product_id`";
	...
	function save($row = array(), $use_mysql_real_escape_string = 1) {
		...
		$this->image_watermark = FSInput::get ('image_watermark');
		...
		$parent_id_name = FSInput::get('parent_id_name');
		if($parent_id_name){
			$row ['parent_id_name'] = $parent_id_name;
			$row ['parent_id'] = FSInput::get('parent_id');
		}else{
			$row ['parent_id_name'] = '';
			$row ['parent_id'] = '';
		}
		...
		$import_price = FSInput::get ('import_price');
		$price_pack = FSInput::get ('price_pack');
		$price_wholesale = FSInput::get ('price_wholesale');
		$price_min = FSInput::get ('price_min');

		$price_old = $this -> standart_money($price_old, 0);
		$price = $this -> standart_money($price, 0);
		$import_price = $this -> standart_money($import_price, 0);
		$price_pack = $this -> standart_money($price_pack, 0);
		$price_wholesale= $this -> standart_money($price_wholesale, 0);
		$price_min= $this -> standart_money($price_min, 0);

		if($id){
			$data = $this->get_record('id = '.$id,'fs_products','price,code');
			if($data->price != $price){
				$row2 = array();
				$row2['subject'] = "Thay đổi giá SKU ".$data->code;
				$row2['message'] = "Giá bán của SKU ".$data->code. " đã được thay đổi từ ".format_money($data->price). " sang ".format_money($price);
				$row2['created_time'] = date('Y-m-d H:i:s');
				
				$this->_add($row2,'fs_messages');
			}
		}
		...
		$row ['price_pack'] = $price_pack;
		$row ['price_wholesale'] = $price_wholesale;
	...
	function memory_related($record_id) {
		if (! $record_id)
			return true;
		
		$record_memory = FSInput::get('memory_record_related',array(),'array');

		if(count($record_memory)){
			$record_memory = array_unique($record_memory);
			foreach ($record_memory as $item) {
				$id_exist = FSInput::get ( 'id_memory_' . $item );
				$name_relate = FSInput::get('name_show_'.$item);

				$row1['name']=$name_relate;
				$row1['record_id']=$record_id;
				$row1['product_relate']=$item;
				$get_relate = $this->get_record('id='.$item,'fs_products','name');
				$row1['name_relate']=$get_relate->name;
				
				$get_r = $this->get_record('record_id ='.$record_id.' AND product_relate='.$item,'fs_products_memory');
				if(!empty($get_r)){
					$u=$this->_update ( $row1, 'fs_products_memory', ' id=' . $get_r->id );
					
				}else{
					$rs = $this->_add ( $row1, 'fs_products_memory');
				}
	...
	function get_memory_related($id){
		if(!$id)
			return;
		$query   = " SELECT * 
		FROM fs_products_memory
		WHERE record_id = ".$id."
		ORDER BY name
		";
		global $db;
	...
	/***********************************/
	function save_exist_memory_related($record_id) {
		...
		foreach ($record_memory as $item){
			...
			// echo $id_exist;
			// die;
			$name_show = FSInput::get ( 'name_show_' . $item );

			$row ['record_id'] = $record_id;
			$row ['name'] = $name_show;

			$row ['product_relate'] = $item;
			$get_relate = $this->get_record('id='.$item,'fs_products','name');
			$row['name_relate']=$get_relate->name;
			$get_r = $this->get_record('product_relate='.$item,'fs_products_memory');	
			
			$u = $this->_update ( $row, 'fs_products_memory', ' id=' . $id_exist );
			if ($u)
				$rs ++;
	...
	function remove_memory_related($record_id) {
		
		$rs = 0;	
		global $db;
		
		$memory_record_related= FSInput::get('memory_record_related',array(),'array');	
		$str_other_memory = implode ( ',', $memory_record_related );
		$whewe ='';
		if ($str_other_memory) {
			$whewe .=' AND product_relate NOT IN ('.$str_other_memory.')';
		}
	// remove in database
		$sql = " DELETE FROM fs_products_memory
	...
	function getManufactories_tree($tablename) {
		$where = '';
		if ($tablename) {
			$where .= 'OR tablenames like "%,' . $tablename . ',%"';
		}
		global $db;
		$query = ' SELECT id,name,parent_id AS parent_id
		FROM fs_manufactories 
		WHERE tablenames is NULL
		' . $where . '	OR tablenames="" ';
		
		$db->query ( $query );
		$manufactories = $db->getObjectList ();
		
		$tree = FSFactory::getClass ( 'tree', 'tree/' );
		$rs = $tree->indentRows ( $manufactories, 1 );
		return $rs;
...
//					return false;
	...
	function ajax_get_memory_related(){
		...
		$manu_id = FSInput::get('manu',0,'int');
		$category_id = FSInput::get('category_id',0,'int');
 ...
 function ajax_get_products_compare(){
 	...
 	function change_extend_image() {
		...
		$row ['group_id'] = $value;
		// $extend_price = $this-> get_record('id = '.$value,'fs_products_price_extend','*');
		// if(!empty($extend_price)) {
		// 	$row ['extend_id'] = $extend_price-> extend_id;
		// }
		...
		foreach ( $this->arr_img_paths_other as $item ) {
			...
			if($this->image_watermark){
				if($item[0] == "resized" || $item[0] == "large" || $item[0] == "compress"){
					$fsFile->add_logo($path_resize,$file_name,PATH_BASE.str_replace('/',DS, 'images/mask/mask_'.$item[0].'.png'),$this->image_watermark_pos);
					$fsFile->add_logo($path_resize,$file_name,PATH_BASE.str_replace('/',DS, 'images/mask/mask_'.$item[0].'_1.png'),$this->image_watermark_pos1);
				}
			}
...
function upload_other_slideshow_highlight() {
		...
		// END uploadAjaxVideoReview
			...
			function ajax_get_tags(){
				...
				if($tags_arr[count($tags_arr) - 1] != ''){
					...
					$get_data = $this->get_records('name like "%' .trim($tags_arr[count($tags_arr) - 1]).'%"','fs_products_tags');
	...
	function reset_amount_hold(){
		$where = "";
		if(!empty($_SESSION [$this->prefix . 'filter1'])){
			$filter = $_SESSION [$this->prefix . 'filter1'];
			if($filter) {
				$where .= ' AND warehouses_id  =' . $filter . '';
			}
		...
		global $db;
		// $str_ids = implode(',',$ids);
		$sql = " UPDATE fs_warehouses_products
		SET amount_hold = 0
		WHERE 1=1 " . $where;
		$rows = $db->affected_rows($sql);
		return $rows;
	...
	function get_bill_transfer_new(){
	    global $db;
	    
	    $query = " SELECT id
				FROM   fs_warehouses_bill_transfer
				ORDER BY id desc LIMIT 1";
				
		$sql = $db->query ( $query );
		
		$check_id_bill_transfer_new = $db->getResult();
		
	    
	   // $check_id_bill_transfer_new = 
	    $query = " SELECT *
				FROM   fs_warehouses_bill_transfer_detail
				WHERE 1=1 AND bill_id = ".$check_id_bill_transfer_new;
		$db->query ( $query );
		
		$result = [];
		
		$getdata = $db->getObjectList ();	
		
// 		foreach($getdata as $key=> $val){
		    
		    
		    
// 		    $result[$key]['product_id'] = $val->amount;
