<?php 
class WarehousesModelsDamaged_products extends FSModels
{
	var $limit;
	var $prefix ;
	...
	{
		$this -> limit = 20;
		$this -> view = 'damaged_products';
		// $this->table_category_name = 'fs_warehouses_categories';

		$this -> table_name = 'fs_warehouses_damaged_products';
		$this -> table_name_detail = 'fs_warehouses_damaged_products_detail';
			// config for save
		...
		$this-> path_excel =  PATH_BASE.LINK_AMIN.DS.'import'.DS.'excel'.DS.'damaged_products'.DS;
		$this-> path_excel_name =  LINK_AMIN.'/import/excel/damaged_products/';
	...
	function setQuery() {
		...
		{
			$text_products = $_SESSION[$this -> prefix.'text0'];
			if($text_products){
				$where .= ' AND ( a.product_name LIKE "%'.$text_products.'%"  OR a.product_code LIKE "%'.$text_products.'%") ';
			}
		}
		...
		$query = " SELECT a.*, p.name, p.code, p.price
		FROM 
		" . $this->table_name_detail . " AS a
		INNER JOIN fs_products as p ON a.product_id = p.id
		...
		if(!empty($list_products)) {
			...
			foreach ($list_products as $product) {
				...
				if(!empty($check_exist)) {
					...
					$row = array();
					$row['amount'] = $check_exist-> amount - $product-> damaged;
					...
					$row['warehouses_name'] = $warehouses-> name;
					$row['amount'] = -$product-> damaged;

					$pro = $this-> get_record('id = '.$product-> product_id,'fs_products','id,name,code');
	...
	function add_products_detail($id){
		...
		foreach ($arr as $idp) {
			...
			$damaged = FSInput::get('ajax_products_damaged_'.$idp);
			...
			$row['create_username'] = $_SESSION ['ad_username'];
			$row['create_userid'] = $_SESSION ['ad_userid'];
			...
			if($warehouses_id) {
				$warehouses = $this-> get_record('id = '.$warehouses_id,'fs_warehouses','id,name');
				$row['warehouses_name'] = $warehouses-> name;
				$row['warehouses_id'] = $warehouses-> id;
			}

			$total_product ++;
			$total_amount += $damaged;
