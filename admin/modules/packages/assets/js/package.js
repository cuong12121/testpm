...
function is_not_package(id){
	if(confirm('Bạn có chắc chắn xác nhận chuyển về trạng thái chưa đóng (ok Sẽ cộng lại tiền đơn này cho tài khoản) ?')){
		$.ajax({
			url: "/admin/index.php?module=packages&view=package&task=ajax_back_package&raw=1",
			data: {id:id},
			dataType: "json",
			success: function(data) {
