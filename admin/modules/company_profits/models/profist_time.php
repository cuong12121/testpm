<?php 
	class Company_profitsModelsProfist_time extends FSModels
	{
		var $limit;
		var $page;
		function __construct()
		{
			...
			$this->limit = $limit;
			$this -> table_name = 'fs_profits_company';
			parent::__construct();
		}
		...
		function save($row = array(), $use_mysql_real_escape_string = 1) {
			...
			if($id && $file_xlsx){
				...
				for($j=2;$j<=$heightRow;$j++){
					...
					$row2['record_id'] = $id;
					$this->_add($row2,'fs_profits_company_cost');
				}
			}

			$chi_phi_khac = $this->get_record('record_id = '.$id,'fs_profits_company_cost','SUM(money) as money');
		...
		function remove_days($record_id) {
			...
			if ($str_other_days) {
				global $db;
				$sql = " DELETE FROM fs_profits_company_cost
		...
		function save_exist_days($id) {
			...
			for ($i = 0; $i < $days_exist_total; $i++) {
				...
				if($id_days_exist){
					...
					$u = $this->_update($row, 'fs_profits_company_cost', ' id=' . $id_days_exist);
					if ($u)
						$rs ++;
				}		
			}
		...
		function save_new_days($record_id){
			...
			for($i = 0; $i < 20; $i ++) {
				...
				$rs = $this->_add ( $row, 'fs_profits_company_cost', 1 );
			}
			return true;
		}

		function get_days($id){
			return $this -> get_records('record_id = '.$id,'fs_profits_company_cost');
		}
