<?php
class Manager_import_productControllersForecast extends Controllers
{
	...
	function display()
	{
		...
		$breadcrumbs = array();
		$breadcrumbs[] = array(0=>'Yêu cầu theo dự báo', 1 => '');	
		$employees = $model -> get_records('group_id = 3','fs_users','*','','','id');
		global $tmpl;
	...
	function add()
	{
		...
		if($product_id){
			$product = $model -> get_record('id = '.$product_id,'fs_products','name,code');
		}
