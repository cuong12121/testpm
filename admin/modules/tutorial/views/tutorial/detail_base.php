...
<?php if(!empty($data)){
	$link = FSRoute::_('index.php?module=tutorial&view=tutorial&code='.$data -> alias.'&ccode='.$data->category_alias.'&id='.$data->id.'&cid='.$data->category_id);
	$link .= '?preview=1';
?>	
...
<?php
	...
	TemplateHelper::dt_edit_text(FSText :: _('Tên 2'),'name_core',@$data -> name_core,'',60,1,0,FSText::_("Không được để trống"));
	...
	TemplateHelper::dt_edit_image(FSText :: _('Image'),'image',str_replace('/original/','/resized/',URL_ROOT.@$data->image),200,100);
	...
	//TemplateHelper::dt_checkbox(FSText::_('Loại bài'),'style',@$data -> style,0,$array_value = array(1 => 'Review', 0 => 'Thường' ));
	//TemplateHelper::dt_checkbox(FSText::_('Kiểu hiển thị rate'),'style_rate',@$data -> style_rate,0,$array_value = array(1 => 'Phần trăm', 0 => 'Sao (đánh giá)' ));
	//TemplateHelper::dt_checkbox(FSText::_('Vị trí rating'),'rate_position',@$data -> rate_position,0,$array_value = array(1 => 'Trên', 0 => 'Dứới' ));

	//TemplateHelper::dt_edit_text(FSText :: _('Tiêu đề đánh giá'),'title_review',@$data -> title_review,'',60,1,0,FSText::_("Không nhập sẽ lấy tiêu đề bài viết"));

	// TemplateHelper::dt_edit_text(FSText :: _('Nội dung ô đánh giá'),'gift_review',@$data -> gift_review,'',60,1,0,FSText::_("Quà tặng, voucher..."));

	//TemplateHelper::dt_edit_text(FSText :: _('Nội dung ô đánh giá'),'gift_review',@$data -> gift_review,'',650,450,1,FSText::_("Quà tặng, voucher..."));

	//TemplateHelper::dt_edit_text(FSText :: _('Tên nút gửi'),'review_btn_text',@$data -> review_btn_text,'',60,1,0);
	//TemplateHelper::dt_edit_text(FSText :: _('Link nút gửi'),'review_btn_link',@$data -> review_btn_link,'',60,1,0);
