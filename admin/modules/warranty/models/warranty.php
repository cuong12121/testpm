...
class WarrantyModelsWarranty extends FSModels
{
	...
	function setQuery() {
		...
		if (isset ( $_SESSION [$this->prefix . 'keysearch'] )) {
			if ($_SESSION [$this->prefix . 'keysearch']) {
				...
				$where .= " AND ( a.tracking_code LIKE '%" . $keysearch . "%' OR a.code LIKE '%" . $keysearch . "%' OR a.phone_guest = '".$keysearch."' )";
			}
		}
	...
	function save($row = array(), $use_mysql_real_escape_string = 0) {
		...
		// if(!$name){
		// 	setRedirect(FSRoute::_($link),'Chưa nhập tên công việc !','error');
		// }
		if($_SESSION['ad_groupid'] == 1){
