<?php 
	class UtilitiesModelsUtilities extends FSModels
	{
		var $limit;
		var $prefix ;
		function __construct()
		{
			$this -> limit = 10;
			$this -> view = 'utilities';
			$this->table_category_name = 'fs_utilities_categories';
			
			$this -> table_name = 'fs_utilities';
			// config for save
			$this -> arr_img_paths = array(array('small',58,58,'cut_image'),array('resized',150,150,'cut_image'),array('large',400,400,'cut_image'),array('compress',1,1,'compress'));
			$this -> img_folder = 'images/utilities';
			$this -> check_alias = 0;
	...
	function get_utilities_categories_tree() {
		global $db;
		$sql = " SELECT id, name, parent_id AS parent_id 
				FROM fs_utilities_categories
