<?php
	function view_comment($controle,$new_id){
		$link = 'index.php?module=tutorial&view=comments&keysearch=&text_count=1&text0='.$new_id.'&filter_count=1&filter0=0';
		return '<a href="'.$link.'" target="_blink">Comment</a>'; 
	}
	function view_history($controle,$new_id) {
		$link = 'index.php?module=tutorial&view=history&news_id=' . $new_id;
		return '<a href="' . $link . '" target="_blink"><img border="0" src="templates/default/images/clock_red.png" alt="History"></a>';
	}
	
	class TutorialControllersTutorial extends Controllers
	{
...
//			$tags_categories = $model->get_tags_categories();
	...
	function unis_hot()
	{
		...
		function change_other_image_content(){
			$this->model->change_other_image_content();
		}

		function getAjaxImagespnContent(){
			$listImagesContent = $this->model->getAjaxImagespnContent();
			// echo '<pre>';
			// print_r($listImagesContent);
			// die;
			include 'modules/' . $this->module . '/views/' . $this->view . '/detail_pn_images_list_content.php';
		}
		function delete_other_image_Content() {
			$this->model->delete_other_image_Content();
		}
	...
	function view_name($controle,$id){
		$model = $controle -> model;
		$data = $model->get_record('id = ' .$id,'fs_tutorial','id,alias,published,category_alias');
