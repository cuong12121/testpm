...
<div class="user_content">
	<div class="head_content">Lịch sử đơn hàng</div>
	<div class="list_history_order">
		<?php if(!empty($list_orders)) { ?>
					...
					<div class="code_order cls"><a title="Xem chi tiết" href="/ket-thuc-don-hang-<?php echo $order-> id ?>.html" target="_blank"><span class="code"><?php echo FSText::_('Mã đơn hàng'); ?> : DH<?php echo str_pad($order -> id, 8 , "0", STR_PAD_LEFT);?></span></a>
						...
						<div class="bottom">
							...
							<?php if($order -> code_sale){ ?>
								...
								<?php }?>
								<?php if($order-> price_ship) { ?>
									<p><span><?php echo FSText::_('Phí ship: '); ?>: </span><span><?php echo format_money($order -> price_ship);?></span></p>
								<?php }?>
								<p><span><?php echo FSText::_('Thanh toán'); ?>: </span><span><?php echo format_money($order -> total_after_discount - $order -> user_point*1000 + $order -> price_ship); ?></span></p>		
							<?php } else {?>
								<?php if($order-> user_point) { ?>
									<p><span><?php echo FSText::_('Dùng điểm: '); ?>: </span><span><?php echo $order -> user_point.' điểm';?></span></p>
									<?php if($order-> price_ship) { ?>
										<p><span><?php echo FSText::_('Phí ship: '); ?>: </span><span><?php echo format_money($order -> price_ship);?></span></p>
									<?php }?>
									<p><span><?php echo FSText::_('Thanh toán'); ?>: </span><span><?php echo format_money($total - $order -> user_point*1000 + $order -> price_ship); ?></span></p>	
								<?php } else {?>
									<?php if($order-> price_ship) { ?>
										<p><span><?php echo FSText::_('Phí ship: '); ?>: </span><span><?php echo format_money($order -> price_ship);?></span></p>
									<?php }?>
			...
			<?php } else {?>
				<div class="item_oder">
					<div class="code_order ">Bạn chưa có đơn hàng nào! Hãy tiếp tục mua sắm.</div></div>
				<?php } ?>
			</div>
