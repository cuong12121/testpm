...
//			$db->query_limit ( $query, $this->limit, $this->page );
		...
		function get_parents_by_ids($parent_ids){
			if (! $parent_ids)
				return;
			return $this -> get_records(' id IN ('.$parent_ids.') ',"fs_".$this->module."_comments",'name,created_time,id,email,comment,parent_id,level,record_id,is_admin,avatar','created_time DESC');
			
		}
		function get_children_by_parents($parent_ids){
			if (! $parent_ids)
				return;
			return $this -> get_records(' parent_id IN ('.$parent_ids.') ',"fs_".$this->module."_comments",'name,created_time,id,email,comment,parent_id,level,record_id,is_admin,avatar','created_time DESC');
